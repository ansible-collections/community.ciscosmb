...
class TestCiscoSMBFactsModule(TestCiscoSMBModule):
    ...
    def load_fixtures(self, commands=None):
        def load_from_file(*args, **kwargs):
            ...
            for command in commands:
                ...
                output.append(
                    load_fixture("ciscosmb_facts-CBS350-24P-4G-%s" % filename)
                )
            return output
    ...
    def test_ciscosmb_facts_default_version(self):
        ...
        self.assertEqual(result["ansible_facts"]["ansible_net_version"], "3.0.0.61")
        self.assertEqual(result["ansible_facts"]["ansible_net_hw_version"], "V01")

    def test_ciscosmb_facts_default_system_info(self):
        with set_module_args(dict(gather_subset="default")):
            result = self.execute_module()

        self.assertEqual(result["ansible_facts"]["ansible_net_uptime"], 1833986)
        self.assertEqual(result["ansible_facts"]["ansible_net_hostname"], "sw-example")
    ...
    def test_ciscosmb_facts_default_inventory(self):
        ...
        self.assertEqual(result["ansible_facts"]["ansible_net_model"], "CBS350-24P-4G")
        self.assertEqual(
    ...
    def test_ciscosmb_facts_hardware(self):
        ...
        self.assertEqual(result["ansible_facts"]["ansible_net_spacefree_mb"], 122.9)
        self.assertEqual(result["ansible_facts"]["ansible_net_spacetotal_mb"], 218.4)
