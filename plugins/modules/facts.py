...
class Default(FactsBase):
    ...
    def parse_inventory(self, data):
        ...
        for line in lines:
            # remove extra chars
            line = re.sub(r'"', r"", line, 0, re.M)
            line = re.sub(r"\s+", r" ", line, 0, re.M)
            # normalize lines
            line = re.sub(r":\s", r'"', line, 0, re.M)
            line = re.sub(r'\s+DESCR"', r'"DESCR"', line, 0, re.M)
            line = re.sub(r'\s+PID"', r'"PID"', line, 0, re.M)
            line = re.sub(r'\s+VID"', r'"VID"', line, 0, re.M)
            line = re.sub(r'\s+SN"', r'"SN"', line, 0, re.M)
            line = re.sub(r"\s*$", r"", line, 0, re.M)
